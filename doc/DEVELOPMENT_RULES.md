...
## ディレクトリ構成のルール (Directory Structure Rules)

### 1. シミュレーションツール
*   バランス調整やデバッグに使用するシミュレータスクリプト（HeadlessGameなど）は、 `tools/simulation` ディレクトリで管理してください。
*   `scripts` ディレクトリには、ビルドやデプロイなどの開発支援スクリプトのみを配置してください。
