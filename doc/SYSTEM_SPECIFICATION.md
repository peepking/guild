...
| **PUBLIC_RELATIONS** | 広報部 | 5 | 1000 * Lv | RECRUIT | 加入 | 冒険者加入率向上。Lv5で以前の標準確率。 |
| **ADMINISTRATION** | 管理部 | 5 | 1000 * Lv | QUEST | 依頼 | 1日の依頼発生数増加。`Count = 2 + Lv`。 |
...
### 2.4 ギルドランク (Guild Rank)
評判(Reputation)に応じてギルドの格付けが決まります。
ギルドランクは、**受注可能な依頼の最大ランク**に影響します。
...
| **C** | 有力ギルド | 200 | Cランクまで |
| **B** | 上級ギルド | 500 | Bランクまで |
| **A** | 英雄級ギルド | 1000 | Aランクまで |
| **S** | 伝説のギルド | 3000 | Sランクまで (全開放) |
...
| **BARD** (吟遊詩人) | 支援/交渉 | - | CHA |
| **SAMURAI** (侍) | アタッカー | HUNT, TOURNAMENT | STR, DEX |
| **SPELLBLADE** (魔法戦士) | アタッカー/魔法 | HUNT, MAGIC | STR, MAG |
| **DARK_KNIGHT** (暗黒騎士) | アタッカー/タンク | HUNT, GUARD | STR, VIT |
| **FENG_SHUI** (風水術師) | 支援/魔法 | MAGIC | INT, MAG |
| **PALADIN** (聖騎士) | タンク/回復 | GUARD, MAGIC | VIT, INT |
| **HUNTER** (狩人) | アタッカー/探索 | EXPLORE, HUNT | STR, DEX |
| **NINJA** (忍者) | 探索/支援 | EXPLORE | DEX, INT, CHA |
| **MARTIAL_ARTIST** (武闘家) | アタッカー/タンク | HUNT, TOURNAMENT | STR, VIT, DEX |
| **BISHOP** (司祭) | 支援/回復 | MAGIC, GUARD | INT, MAG, CHA |
...
### 3.6 奥義 (Ultimate Arts)
高ランク冒険者が習得する必殺技システム。

*   **習得条件**: ランク **B** (380) および **S** (1000) への昇格時にそれぞれランダムで1つ習得。
*   **種類**: クラスごとに5種類の奥義が存在（16クラス × 5 = 全80種）。
    *   例: 戦士の「回転切り」、斥候の「暗殺撃」、商人の「銭投げ」など。
*   **効果**: 1つ習得するごとに **戦闘総合力(CP) +6%** 加算相当（Cランク装備1つ分）。
*   **戦闘演出**:
    *   戦闘ログにおいて、**30%** の確率で奥義専用ログが表示される。
    *   奥義不使用時（または未習得者）は、クラス固有の通常行動ログが表示される。
    *   パーティメンバー全員分の行動ログが表示され、戦闘の臨場感が高まる。

### 3.7 装備 (Equipment)
冒険者は武器と防具を装備し、ステータス(CP)にボーナスを得る。

*   **武器タイプ**:
    *   **LONG_SWORD (長剣)**: 戦士、侍、騎士向け。平均的な攻撃力。
    *   **SHORT_SWORD (短剣)**: 斥候、忍者向け。高命中。
    *   **AXE (斧)**: 戦士、暗黒騎士向け。高威力だが命中難。
    *   **MACE (槌)**: 僧侶、聖騎士、司祭向け。
    *   **STAFF (杖)**: 魔導士、風水術師向け。魔法威力強化。
    *   **BOW (弓)**: 狩人、斥候向け。後列攻撃。
    *   **SPEAR (槍)**: *[New]* 暗黒騎士、騎士、聖騎士向け。
    *   **GAUNTLET (ガントレット)**: *[New]* 武闘家、戦士、斥候向け。格闘戦用武器。
    *   **SPECIAL (特殊)**: 魔法道具、暗器など。
*   **防具タイプ**:
    *   **HEAVY (重装)**: 戦士、騎士、暗黒騎士、聖騎士。
    *   **LIGHT (軽装)**: 斥候、狩人、武闘家。
    *   **CLOTHES (服)**: 吟遊詩人、侍、忍者、商人。
    *   **ROBE (ローブ)**: 魔導士、僧侶、風水術師、司祭。
...
### 4.1 基本仕様
*   **発生数**: 管理部(Administration)のレベルに依存 (`2 + Lv`)。
    *   初期: 2件 / 最大(Lv5): 7件 (コード上は上限10まで許容)
...
#### 特殊・その他
*   **TOURNAMENT (天下一武闘会)**: 冒険者の実力を競う大会。
    *   **個人戦 (SOLO)**: 1vs1のトーナメント。勝利するごとにランク(E->S)が上がる。
    *   **団体戦 (TEAM)**: 4人パーティ推奨の団体戦。対戦相手は他地域のギルド代表チーム。
    *   **仕様**: 期間4日。参加費無料。固定報酬のみ（討伐報酬・ドロップなし）。
*   **NEGOTIATE (交渉)**: 商人・吟遊詩人・貴族などが活躍。借金取り立て、詐欺調査、喧嘩の仲裁など、戦闘よりも対話スキル(CHA/INT)が重視される依頼。
*   **SPECIAL (特務)**: 特定条件（資料室Lv等）で発生。Sランク級ボスや古代種、異界の魔物など。ギルドの総力を挙げた決戦。
...
### 5.2 顧問 (Advisors)
引退した高ランク冒険者(B以上)や、外部から招聘した専門家を「顧問」として雇用し、ギルド全体にパッシブ効果を与えるシステム。

*   **雇用形態**:
    *   **Appointment (任命)**: 所属するランクB以上の冒険者を引退させ、顧問に転身させる。
    *   **Headhunt (招聘)**: 資金(2000G)を支払い、外部からランクB相当の専門家を90日間契約で雇う。
*   **コスト**: 1名につき 100G / 30日。
*   **上限**: 最大20名。
*   **効果 (主な例)**:
    *   **戦術顧問 (TACTICIAN)**: 勝率UP (INT依存, 魔術師系)
    *   **技術教官 (INSTRUCTOR)**: 経験値UP (DEX依存, 武闘家系)
    *   **警備隊長 (GUARD_CAPTAIN)**: 負傷率DOWN (VIT依存, 戦士系)
    *   **同職重複時の減衰**: 同じ効果を持つ顧問を複数雇った場合、効果は累積するが2人目以降は効果量が半減する(100% -> 50% -> 25%...)。
...
### 6.1 セーブシステムの概要
ゲームの進行状況は、プレイヤーのブラウザ領域（LocalStorage）に保存される。

### 6.2 仕様詳細
*   **オートセーブ (Auto-Save)**:
    *   **タイミング**: 日付変更処理（Next Day）完了時に自動実行。
    *   **保存先**: LocalStorage (`guild_master_save_v1`)。
    *   **内容**: ギルド状態、冒険者、クエスト進行状況、履歴、メール等すべてのゲームステート。
*   **オートロード (Auto-Load)**:
    *   ゲーム起動（ページ読み込み）時に、有効なセーブデータが存在すれば自動的に復元する。
*   **手動エクスポート/インポート**:
    *   運営メニューの「システム」タブから利用可能。
    *   **Export**: 現在のセーブデータをBase64エンコード文字列として出力（クリップボードへコピー）。
    *   **Import**: エクスポートされた文字列を入力してデータを復元（上書き）。
*   **リセット**:
    *   ローカル保存データを削除し、ゲームを初期状態（Day 1）に戻す機能。
